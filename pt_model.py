...
class PtModel:
    ...
    def _setup_nodes(self):
        self._logger.debug("Setting up nodes.")
    ...
    def _setup_elements(self):
        self._logger.debug("Setting up elements.")
    ...
    def _setup_model(self):
        ...
        if self._un == [[]]:
            self._logger.info("Displacements are not set.")
    ...
    def set_beampars(self, element, beampars, values):
        self._logger.debug("Setting beam parameters. Beam(%s): %s = %s", element, beampars, values)
        ...
        if len(beampars) == len(values):
            for idx, par in enumerate(beampars):
                ...
                match par:
                    ...
                    case _:
                        self._logger.error("Beam parameters not set. %s", par)
                        raise ValueError("Beam parameters not set. %s" % par)

            self._logger.debug("Beam parameters set. %s", beam3d_pars)
            self.elements[element - 1] = (beam3d(self.nodes, beam3d_pars))

        else:
            self._logger.error("Beam parameters and values shape mismatch. Beam parameters shape: %s, Values shape: %s",
                               np.shape(beampars), np.shape(values))
            raise ValueError(
                "Beam parameters and values shape mismatch. Beam parameters shape: %s, Values shape: %s" % (
                    np.shape(beampars), np.shape(values)))

    def clear_constraints(self):
        self._logger.debug("Clearing constraints.")
        # Clear the constraints for the model
        # Set the constraints for the model
        # t - time [s]
        # u - displacement [mm]
        mp = model.extract_pars()
        self._logger.debug("Extracting model parameters. %s", mp)
        self.model.dofs_c = 0
        self._c = np.zeros((self.nodes.n_nodes, 3))

    def set_constraints(self, t, nodes, direction):
        self._logger.debug("Setting constraints. t: %s, nodes: %s, direction: %s", t, nodes, direction)
        # Set the constraints for the model
        # t - time [s]
        # u - displacement [mm]
        i, n = np.shape(nodes)
    ...
    def get_constraints(self, nodes, direction):
        self._logger.debug("Getting constraints. nodes: %s, direction: %s", nodes, direction)
    ...
    def calculate_fatigue(self, cycles):
        self._logger.debug("Calculating fatigue...")
        D = 0.0
        ...
        self._logger.debug("Fatigue damage: %s, E-Module %s", D, E)
        return [D, E]

    def clear_displacement(self, node, direction):
        # Clear the load for the model
        self._logger.debug("Clearing displacement. node: %s, direction: %s", node, direction)
        if direction == 0:
            for d in range(3):
    ...
    def clear_displacements(self):
        self._logger.debug("Clearing displacements.")
    ...
    def set_displacements(self, u, nodes, direction):
        ...
        if np.shape(u) == np.shape(nodes) == np.shape(direction):
            ...
            self._logger.error(
                "Displacement, node and direction shape mismatch. Displacement shape: %s, Node shape: %s, Direction shape: %s",
                np.shape(u), np.shape(nodes), np.shape(direction))
            raise ValueError(
                "Displacement, node and direction shape mismatch. Displacement shape: %s, Node shape: %s, Direction shape: %s" % (
                    np.shape(u), np.shape(nodes), np.shape(direction)))
    ...
    def set_displacements_between_nodes(self, U, nodes):
        self._logger.debug("Setting displacements between nodes. u: %s, nodes: %s", U, nodes)
        ...
        if np.shape(U)[0] == np.shape(nodes)[0] and n == 2:
            ...
            for _i in range(i):
                ...
                if not len(self.BTW_U) == 0:
                    BTW_idx = \
                        np.where(
                            (node1 == np.array(self.BTW_U_nodes)[:, 0]) & (node2 == np.array(self.BTW_U_nodes)[:, 1]))[
                            0]
                else:
                    BTW_idx = []
                ...
                if len(BTW_idx) == 0 or len(self.BTW_U) == 0:
                    ...
                    if not U1 == 0:
                        self._logger.debug("Correcting scale")
                        scale = scale * (U0 / U1)
                    if abs(U0 - U1) > U[-i] * 0.1:
                        self._logger.warning("U0: %s exceeds U1: %s with more than 10% of U: %s", U0, U1, U[_i])
                ...
                if isnan(F):
                    self._logger.warning("Force is NaN. %s", F)
                    F = 1  # default force [N]
                self._logger.debug("Force needed for set displacement. %s", F)

                try:
                    self.set_loads_between_nodes(F, nodes[_i])
                except Exception as e:
                    self._logger.error("Error setting loads between nodes: %s", e)
                    raise

        else:
            self._logger.error("Displacement and node shape mismatch. Displacement shape: %s, Node shape: %s",
                               np.shape(U), np.shape(nodes))
            raise ValueError("Displacement and node shape mismatch. Displacement shape: %s, Node shape: %s" % (
                np.shape(U), np.shape(nodes)))
    ...
    def get_displacement(self, nodes, direction):
        ...
        if len(noisy_displacements) == 1:
            noisy_displacements = noisy_displacements[0]
    ...
    def get_distance_between_nodes(self, node1, node2):
        ...
        L1 = sqrt((xyz1[0] - xyz2[0] + ulok[0]) ** 2 + (xyz1[1] - xyz2[1] + ulok[1]) ** 2 + (
                xyz1[2] - xyz2[2] + ulok[2]) ** 2)  # length [mm]
        delta_l = L1 - L0  # deltaL [mm]

        self._logger.debug("L0: %s, L1: %s, DeltaL: %s", L0, L1, delta_l)
        return L0, L1, delta_l

    def get_displacements(self):
        # Get the displacements for the model
        self._logger.debug("Getting all displacements.")
        return self.u[:, 1]

    def clear_load(self, node, direction):
        # Clear the load for the model
        self._logger.debug("Clearing load. node: %s, direction: %s", node, direction)
        if direction == 0:
            for d in range(3):
    ...
    def clear_loads(self):
        ...
        self._logger.debug("Clearing loads.")
    ...
    def set_loads(self, f, nodes, direction):
        ...
        if np.shape(f) == np.shape(nodes) == np.shape(direction):
            ...
            self._logger.error(
                "Load, node and direction shape mismatch. Load shape: %s, Node shape: %s, Direction shape: %s",
                np.shape(f), np.shape(nodes), np.shape(direction))
            raise ValueError(
                "Load, node and direction shape mismatch. Load shape: %s, Node shape: %s, Direction shape: %s" % (
                    np.shape(f), np.shape(nodes), np.shape(direction)))
    ...
    def set_loads_between_nodes(self, F, nodes):
        ...
        if isinstance(F, list):
            ...
            self._logger.debug("F is a list. %s", F)
        ...
        if np.shape(F)[0] == np.shape(nodes)[0] and n == 2:
            ...
            for _i in range(i):
                ...
                for d in range(3):
                    try:
                        ...
                        self._logger.error("Error finding dof: %s", e)
                        raise
            ...
            self._logger.error("Loads and node shape mismatch. Load shape: %s, Node shape: %s", np.shape(F),
                               np.shape(nodes))
            raise ValueError(
                "Loads and node shape mismatch. Load shape: %s, Node shape: %s" % (np.shape(F), np.shape(nodes)))

        # self._l.debug("Loads between nodes: %s", self.BTW)
        # self._setup_model()

    def get_load(self, nodes, direction):
        self._logger.debug("Getting loads. nodes: %s, direction: %s", nodes, direction)
        ...
        if np.shape(nodes) == np.shape(direction):
            for _i in range(i):
                ...
                try:
                    ...
                    self._logger.error("Error finding load index: %s", e)
                    fs = [0]
        else:
            self._logger.error("Load and node shape mismatch. Load shape: %s, Node shape: %s", np.shape(nodes),
                               np.shape(direction))
            raise ValueError(
        ...
        if len(noisy_forces) == 1:
            noisy_forces = noisy_forces[0]
