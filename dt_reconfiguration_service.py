...
from communication.shared.protocol import ROUTING_KEY_STATE, ROUTING_KEY_FORCES, ROUTING_KEY_DT_FORCES

class DT_ReconfigurationService:
    ...
    def turn_off_forces(self):
        with Rabbitmq(**config["rabbitmq"]) as rabbitmq:
            ...
            rabbitmq.send_message(ROUTING_KEY_FORCES, {"forces": False}) # we also need to shut off the PT!
    ...
    def query_influxdb(self):
            ...
            flux_query = f'''
                ...
                |> filter(fn: (r) => r["source"] == "dt_stl_monitor")
    ...
    def setup(self):
            ...
            self._l.info(f"DT_ReconfigurationService setup complete.")
...
if __name__ == "__main__":
    ...
    service = DT_ReconfigurationService(rabbitmq_config=config["rabbitmq"], influxdb_config=config["influxdb"])

    service.setup()
    
    # Start the DT_ReconfigurationService
